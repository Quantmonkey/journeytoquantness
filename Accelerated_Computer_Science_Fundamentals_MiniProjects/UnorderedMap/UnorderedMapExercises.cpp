...
StringIntMap makeWordCounts(const StringVec& words) {
  ...
  for (int i=0; i != words.size(); i++) {
    if (wordcount_map.count(words[i])) {
      // wordcount_map contains the key words[i]
      // add a count to the wordcount_map value
      wordcount_map[words[i]]++;
    }
    else {
      // wordcount_map does not contain the key words[i]
      // create a value count of 1 with the key words[i]
